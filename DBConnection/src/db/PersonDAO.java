...
//DAO(Data Accees Object) - 데이터베이스 연결 및 연동(입력, 검색, 수정, 삭제)기능 클래스
public class PersonDAO {
	//필드
	private Connection conn = null; // 연결처리클래스
	...
	public ArrayList<Person> getPersonList(){
		...
		try {
			conn = JDBCUtil.getConnection(); // conn - 연결객체 생성
			...
			while(rs.next()) { // 다음에 가져올 데이터가 있다면
				...
				//person테이블에서 userid를 가져와서 person 객체의 userId에 저장함
		...
		return personList; // 호출하는 곳으로 반환(돌려줌)
	}
